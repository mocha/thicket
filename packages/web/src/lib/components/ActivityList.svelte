        ...
        <li>
          ...
          {:else if e.kind === 'bookmark'}
            <div class="row">
              ...
              <p class="what">
                Saved <a href={webHref(e.payload.url) ?? '#'} target="_blank" rel="noopener">{e.payload.title ?? e.payload.url}</a>
          ...
          {:else}
            <div class="row">
              <SourceIcon feedId={e.payload.feedId} hasIcon={e.payload.hasIcon} name={e.payload.siteTitle ?? e.payload.title} size={20} />
              <p class="what">
                Noted on <a href={webHref(e.payload.url) ?? '#'} target="_blank" rel="noopener">{e.payload.title ?? e.payload.url}</a>
                <span class="src">{e.payload.siteTitle ?? hostOf(e.payload.url)}</span>
              </p>
              <span class="when">{relativeTime(e.at)}</span>
            </div>
