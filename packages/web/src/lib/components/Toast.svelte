...
<style>
  .toast {
    ...
    box-shadow: var(--shadow); font-size: calc(15px * var(--size-app)); z-index: 50;
  }
  @media (min-width: 900px) { .toast { bottom: 20px; } }
  .action { color: var(--accent); font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; font-size: calc(13px * var(--size-app)); }
