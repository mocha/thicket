...
<style>
  .palettes { display: grid; grid-template-columns: repeat(auto-fill, minmax(92px, 1fr)); gap: var(--space-2); }
  .palettes button {
    display: flex; flex-direction: column; gap: var(--space-2); padding: var(--space-2); border-radius: var(--radius-sm);
  ...
  /* Deliberately literal below: this is a tiny drawing of a page (with a card floating
     on it) inside the swatch, not real UI, so its whole layout is sized to the drawing. */
  ...
  /* 2px centers the label under the swatch above it — optical, not spacing. */
